...
{
        BN_clear_free(ecc->bn2);
        BN_clear_free(ecc->bn);
                ...
                struct ntb_buffer *data_out)
{
        ...
        if (data_in_length < iv_length)
                { OPENSSL_cleanse(ecdh_keybuffer, sizeof ecdh_keybuffer); return false; }
        iv = data_in;
        data_in += iv_length;
        ...
        if (CRYPTO_memcmp(hmac_buf, mac, sizeof hmac_buf))
                { OPENSSL_cleanse(ecdh_keybuffer, sizeof ecdh_keybuffer); return false; }
        ...
        OPENSSL_cleanse(ecdh_keybuffer, sizeof ecdh_keybuffer);
        return result;
}
